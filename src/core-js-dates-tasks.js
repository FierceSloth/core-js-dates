...
function getNextFriday(date) {
  ...
  const targetDay = 5;
  let daysToAdd = (7 + targetDay - date.getDay()) % 7;

  if (daysToAdd === 0) {
    daysToAdd = 7;
  }

  return new Date(date.setDate(currentDate + daysToAdd));
}
...
function getCountDaysInMonth(month, year) {
  ...
  const date = new Date(year, trueMonth);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.round((new Date(dateEnd) - new Date(dateStart)) / msPerDay) + 1;
}
 ...
 */
function isDateInPeriod(strDate, period) {
  const { start: strStart, end: strEnd } = period;
...
function formatDate(dateISO) {
  ...
  const leftSide = [month, day, year].join('/');
  let rightSide = [formatedHours, minutes, seconds].join(':');

  rightSide += hours >= 12 ? ' PM' : ' AM';
...
function getCountWeekendsInMonth(month, year) {
  const trueMonth = month - 1;
  const date = new Date(year, trueMonth, 1);
...
function getWeekNumberByDate(date) {
  ...
  const timeDiff = d - yearStart;

  return Math.ceil((timeDiff / msInDay + 1) / 7);
}
...
function getNextFridayThe13th(date) {
  ...
  resultDate.setMonth(currentMonth);
